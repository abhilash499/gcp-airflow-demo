...
update_env_variable() {
  ...
  grep -q "$name" "$3" && sed -i "s?$name.*?$name$value?" "$3" &> /dev/null || echo "$name$value" >> "$3"
}

update_env_variable "export AIRFLOW_HOME=" "${AIRFLOW_HOME}" ~/.bashrc
update_env_variable "export AIRFLOW__CORE__EXECUTOR=" "${AIRFLOW__CORE__EXECUTOR}" ~/.bashrc
update_env_variable "export AIRFLOW__CORE__LOAD_EXAMPLES=" "${AIRFLOW__CORE__LOAD_EXAMPLES}" ~/.bashrc
update_env_variable "export AIRFLOW__CORE__FERNET_KEY=" "${AIRFLOW__CORE__FERNET_KEY}" ~/.bashrc
...
if [ -e "/requirements.txt" ]; then
    $(command -v pip) install --user -r /requirements.txt --no-warn-script-location
fi

# Install packages required for testing
if [ -e "/requirements-test.txt" ]; then
    $(command -v pip) install --user -r /requirements-test.txt --no-warn-script-location
fi

# Install packages required for lint
if [ -e "/requirements-lint.txt" ]; then
    $(command -v pip) install --user -r /requirements-lint.txt --no-warn-script-location
fi

# Install developers custom packages
if [ -e "/requirements-user.txt" ]; then
    $(command -v pip) install --user -r /requirements-user.txt --no-warn-script-location
fi
# install data-pipelines project. For now it's only the scripts that are
# getting installed, but later DAG can also be installed to eliminate polling
# of the file system for changes

$(command -v pip) install --user ${AIRFLOW_HOME}
...
if [ "$AIRFLOW__CORE__EXECUTOR" != "SequentialExecutor" ]; then
  ...
  if [ -z "$AIRFLOW__CORE__SQL_ALCHEMY_CONN" ]; then
    ...
    update_env_variable "export AIRFLOW__CORE__SQL_ALCHEMY_CONN=" "${AIRFLOW__CORE__SQL_ALCHEMY_CONN}" ~/.bashrc
